...
SELECT 
  MAX(attendance) AS maior_publico_em_um_jogo,
  MIN(attendance) AS menor_publico_em_um_jogo,
  AVG(attendance) AS media_de_publico_anual,
  COUNT(attendance) AS jogos_com_publico,
  SUM(attendance) AS publico_total_anual
FROM `bigquery-public-data.baseball.schedules`;
